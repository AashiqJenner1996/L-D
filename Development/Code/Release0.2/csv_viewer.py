        ...
        with open(filename, 'r') as file:
            ...
            for i, column in enumerate(header):
                if column_stats[i]:
                    ...
                    p90 = statistics.quantiles(column_stats[i], n=1000)[900]
                    output_text.insert(tk.END, f"Column: {column}\n")
                    output_text.insert(tk.END, f"Mean: {mean:.2f}\n")
                    output_text.insert(tk.END, f"P50 (median): {p50:.2f}\n")
                    output_text.insert(tk.END, f"P90: {p90:.2f}\n")
...
output_text = ScrolledText(window, height=100, width=400, font=("Times New Roman", 12))
output_text.pack(padx=10, pady=10)

# Set the font for the header rows to bold
output_text.config(font=("Times New Roman", 12, "bold"))

# Display the header rows in bold
for i, column in enumerate(header):
    output_text.insert(tk.END, f"Column: {column}\n", "bold")
